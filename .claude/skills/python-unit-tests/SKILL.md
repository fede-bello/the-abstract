...
- `--import-mode=importlib` is set, so the mirrored layout can reuse basenames (`steps/parsing/test_step.py` and `steps/categorization/test_step.py` coexist). No `__init__.py` needed under `backend/tests/`.
