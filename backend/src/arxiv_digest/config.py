...
class Settings(BaseSettings):
    ...
    # --- API (hosts the /ask endpoint; the frontend reads papers from Supabase directly) ---
    api_host: str = Field(default="127.0.0.1")
    api_port: int = Field(default=8000, gt=0)
    # Browser origins allowed by CORS when the SPA calls /ask; the Vite dev server runs on 5173.
