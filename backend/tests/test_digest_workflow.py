"""Offline end-to-end tests for the classify -> parse -> collect fan-out/fan-in.

The LLM and LlamaParse boundaries are stubbed, so these run with no network and
exercise the workflow's routing: useful papers get parsed and survive, noise papers
are dropped, parse failures are dropped, and an empty run completes cleanly.
"""
