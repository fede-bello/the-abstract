...
There is no API server: the backend is purely the weekly pipeline (writes to Supabase), and the SPA reads Supabase directly. The whole thing hosts on Vercel (static SPA) + Supabase (DB) for free. The pipeline runs weekly via a GitHub Actions cron (`.github/workflows/weekly-digest.yml`) — `arxiv-digest ingest` ingests *and* emails the digest (its last step). Newsletter signup is **double opt-in**: the footer form calls the `subscribe` Supabase Edge Function (Deno, in `supabase/functions/`), which records a pending row and emails a confirmation link; `confirm`/`unsubscribe` Edge Functions handle the emailed links. Those functions use the service role; the anon key can't touch the private `subscribers` table. The three functions are the *only* server-side request handlers in the project — everything else the SPA reads straight from Supabase.
